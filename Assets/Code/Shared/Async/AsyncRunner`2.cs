...
{
    /// <summary>
    /// Cancel-and-restart wrapper around an async action taking 2 arguments: every call aborts the one still
    ...
    /// </summary>
    public class AsyncRunner<T1, T2> : IDisposable
    {
        private readonly Func<T1, T2, CancellationToken, UniTask> _action;
        ...
        /// </summary>
        public AsyncRunner(Func<T1, T2, CancellationToken, UniTask> action, float delay = 0f)
        {
            _action = action;
            _delay = delay;
        }
        ...
        /// </summary>
        public void CallAsync(T1 arg1, T2 arg2)
        {
            CallTaskAsync(arg1, arg2).Forget();
        }

        /// <summary>
        /// Awaitable form: completes when the action finishes, or right away when a newer call cancels this one
        /// </summary>
        public async UniTask CallTaskAsync(T1 arg1, T2 arg2)
        {
            var token = Restart();
