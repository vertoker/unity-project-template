...
{
    /// <summary>
    /// Cancel-and-restart wrapper around an async action: every call aborts the one still
    ...
    public class AsyncRunner : IDisposable
    {
        ...
        /// </summary>
        public AsyncRunner(Func<CancellationToken, UniTask> action, float delay = 0f)
        {
            _action = action;
            _delay = delay;
        }
