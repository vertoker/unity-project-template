...
{
    /// <summary>
    /// Cancel-and-restart wrapper around an async action taking 1 argument: every call aborts the one still
    ...
    /// </summary>
    public class AsyncRunner<T1> : IDisposable
    {
        private readonly Func<T1, CancellationToken, UniTask> _action;
        ...
        /// </summary>
        public AsyncRunner(Func<T1, CancellationToken, UniTask> action, float delay = 0f)
        {
            _action = action;
            _delay = delay;
        }
